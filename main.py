    ...
    try:
        ...
        try:
            ...
            ws = prepare_websocket(
                stream_url,
                header={
                    "Origin": f"https://twitcasting.tv/{user}",
...
def _get_stream_info(user, proxy="", user_agent=""):
    ...
    r = requests.get(
        url, proxies={"http": proxy, "https": proxy}, headers={"User-Agent": user_agent}
    )
    data = r.json()
    return data
...
def get_stream_url(user, proxy="", user_agent=""):
    ...
    try:
        ...
        stream_url = (
            f"{proto}://{host}/ws.app/stream/{movie_id}/fmp4/bd/1/1500?mode={mode}"
        )
    return stream_url
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "user_id",
        help='The user id to record. i.e. the string after "https://twitcasting.tv/" in URL',
    )
